...
func (i *interceptionBase) newMessagesService(ctx context.Context, opts ...option.RequestOption) (anthropic.MessageService, error) {
	...
	if i.bedrockCfg != nil {
		bedrockOpts, err := i.withAWSBedrockOptions(i.bedrockCfg)
		if err != nil {
...
// (see provider.bedrockCredentialCache): either static credentials, the AWS SDK
// default credential chain (environment variables, shared config/credentials
// files, IAM roles, IRSA, SSO, IMDS, etc.), or a role assumed via STS when a
// target role ARN is configured. The cache rotates temporary credentials, so
// this per-request path only assembles request options around it.
func (i *interceptionBase) withAWSBedrockOptions(cfg *aibconfig.AWSBedrock) ([]option.RequestOption, error) {
	if cfg == nil {
