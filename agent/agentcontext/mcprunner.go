...
func (r *mcpRunner) connectAndList(ctx context.Context, cfg mcpServerConfig) ([]MCPTool, error) {
	...
	// Use the parent ctx (not connectCtx) for Start so a stdio
	// subprocess is tied to the runner's lifetime; the deferred Close
	// terminates it once we have the tool list. connectCtx bounds only
	// the initialize and list handshakes below.
...
func (r *mcpRunner) createTransport(ctx context.Context, cfg mcpServerConfig) (transport.Interface, error) {
	...
	case "http", "":
		...
		if hc := mcpHTTPClient(); hc != nil {
			opts = append(opts, transport.WithHTTPBasicClient(hc))
		}
		return transport.NewStreamableHTTP(cfg.URL, opts...)
	case "sse":
		sseOpts := []transport.ClientOption{transport.WithHeaders(cfg.Headers)}
		if hc := mcpHTTPClient(); hc != nil {
			sseOpts = append(sseOpts, transport.WithHTTPClient(hc))
		}
...
// mcpHTTPClient lives in mcphttpclient.go so this file does not import
// "testing" (which the timeout ruleguard lint treats as test code).
