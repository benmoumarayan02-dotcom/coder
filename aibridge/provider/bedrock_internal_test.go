...
// TestBedrockCredentialCacheBuildsOnce verifies the per-provider cache resolves
// the credentials provider once and reuses it across calls.
func TestBedrockCredentialCacheBuildsOnce(t *testing.T) {
	t.Parallel()

	cache := newBedrockCredentialCache(config.AWSBedrock{
		Region:          "us-east-1",
		AccessKey:       "test-key",
		AccessKeySecret: "test-secret",
	})
	...
	// The same cached provider instance is returned, proving the build is
	// memoized rather than repeated per call.
