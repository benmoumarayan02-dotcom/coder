...
parse_beta_features() {
	jq -r '
		# Walk routes top-down. When a node is marked beta, emit it and
		# stop descending; its subtree is implicitly beta. Keep recursing
		# through non-beta ancestors until we find a beta node or run out.
		def walk:
			if (.state // []) | index("beta") then
				.
			else
				.children[]? | walk
			end;
